...
use constants::{Si7021_READ_FW_VERSION, Si7021_READ_HEATER_CONTROL, Si7021_READ_ID_BYTE_1, Si7021_READ_ID_BYTE_2, Si7021_READ_RH_NO_HOLD, Si7021_READ_RH_T_USER_REG_1, Si7021_READ_TEMP_AFTER_PREVIOUS_RH, Si7021_READ_TEMP_NO_HOLD, Si7021_RESET, Si7021_WRITE_HEATER_CONTROL, Si7021_WRITE_RH_T_USER_REG_1};
    ...
    D: DelayNs,
{
    ...
    fn write_command<const N: usize>(&mut self, command_buf: [u8; N] ) -> Result<(), Error<E>> {
    ...
    fn read_register( &mut self, register_address: u8, buffer: &mut [u8] ) -> Result<(), Error<E>> {
    ...
    pub fn is_connected(&mut self) -> Result<bool, Error<E>> {
        ...
        self.read_register(Si7021_READ_RH_T_USER_REG_1, &mut result_buf)?;
    ...
    pub fn reset_device(&mut self) -> Result<(), Error<E>> {
        debug!("in reset_device()");
        self.write_command([Si7021_RESET])?;
    ...
    pub fn init_device(&mut self) -> Result<bool, Error<E>> {
        ...
        if (self.read_device_model()? == DeviceModel::NotRead) {
            debug!("device model is not read");
            return Ok(false);
        }
        self.read_firmware_version()?;
        debug!("fw version is {:#?}", self.read_firmware_version()?);
        Ok(true)
    }
    ...
    pub fn read_firmware_version(&mut self) -> Result<u8, Error<E>> {
        ...
        self.i2c
            .write_read(self.address, &command_buffer, &mut result_buf)
            .map_err(Error::I2c)?;
    ...
    /// read device model 
    pub fn read_device_model(&mut self) -> Result<DeviceModel, Error<E>> {
    ...
    pub fn read_relative_humidity(&mut self) -> Result<f32, Error<E>> {
        debug!("in read_relative_humidity()");
        self.delayer.delay_ms(20);  // in case user called this again too quickly
        let mut command_buffer: [u8; 1] = [Si7021_READ_RH_NO_HOLD]; 
        self.i2c.write(self.address, &command_buffer)
            .map_err(Error::I2c)?;
        self.delayer.delay_ms(25);
        debug!("did i2c write, next is read");
        let mut result_buf: [u8; 3] = [0; 3];
        self.i2c.read(self.address, &mut result_buf)
            .map_err(Error::I2c)?;
    ...
    pub fn read_temperature(&mut self) -> Result<f32, Error<E>> {
        ...
        let mut command_buffer: [u8; 1] = [Si7021_READ_TEMP_NO_HOLD]; 
        self.i2c.write(self.address, &command_buffer)
                .map_err(Error::I2c)?;
        ...
        // scale it
        let mut temperature: f32 = ( temperature_u16 as f32 * 175.72 / 65536.0 ) - 46.85;
    ...
    pub fn read_measurements(&mut self) -> Result<Measurements, Error<E>> {
        ...
        let humidity = self.read_relative_humidity()?;
        //self.delayer.delay_ms(5);
        let mut result_buf: [u8; 2] = [0; 2];
        self.read_register(Si7021_READ_TEMP_AFTER_PREVIOUS_RH, &mut result_buf)?;
        let temperature_u16 : u16 = u16::from_be_bytes( [result_buf[0], result_buf[1] ]);
        // scale it
        let mut temperature: f32 = ( temperature_u16 as f32 * 175.72 / 65536.0 ) - 46.85;
    ...
    pub fn heater_control(&mut self, enable: bool) -> Result<(), Error<E>> {
        ...
        self.write_command([Si7021_WRITE_RH_T_USER_REG_1, write_value])?;
    ...
    pub fn set_heater_level(&mut self, level: u8) -> Result<(), Error<E>> {
        debug!("in set_heater_level({})", level);
        if (level > 0x0f) {
            return Err(Error::OutOfRange(level));
        }
        self.write_command([Si7021_WRITE_HEATER_CONTROL, level])?;
        Ok(())
    }
